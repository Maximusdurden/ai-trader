...
def start_bot():
	...
	logger.info("[BOT] Starting AI Trading Bot (main_hybrid.py)...")
	try:
		...
		logger.info("[OK] Trading Bot started (PID: %d)" % process.pid)
		return process
...
def start_dashboard():
	...
	logger.info("[DASH] Starting Dashboard (dashboard.py)...")
	try:
		process = subprocess.Popen(
		...
		logger.info("[OK] Dashboard started (PID: %d)" % process.pid)
	...
	for service_name, process in processes.items():
		if process and process.poll() is None:
			...
			try:
				...
				logger.error(f"   [ERR] Error stopping {service_name}: {e}")
	logger.info("[OK] All services stopped. Goodbye!")
	sys.exit(0)
...
def run_all():
	...
	if not bot_process and not dashboard_process:
		logger.error("[ERR] Failed to start any services!")
		sys.exit(1)
	...
	if bot_process:
		logger.info("   [BOT] Trading Bot (main_hybrid.py)")
	if dashboard_process:
		logger.info("   [DASH] Dashboard (http://localhost:5000)")
	logger.info("")
	...
	try:
		while True:
			...
			for service_name, process in list(processes.items()):
				if process and process.poll() is not None:
					logger.error(f"[ERR] {service_name} has crashed!")
...
def run_bot_only():
	...
	logger.info("[BOT] Starting Trading Bot Only...")
	bot_process = start_bot()
	if not bot_process:
	...
	try:
		while True:
			...
			if bot_process.poll() is not None:
				logger.error("[ERR] Bot process has exited")
				sys.exit(1)
	except KeyboardInterrupt:
		handle_shutdown(None, None)
...
def run_dashboard_only():
	...
	logger.info("[DASH] Starting Dashboard Only...")
	logger.info("   Open http://localhost:5000 in your browser")
	dashboard_process = start_dashboard()
	...
	try:
		while True:
			...
			if dashboard_process.poll() is not None:
				logger.error("[ERR] Dashboard process has exited")
				sys.exit(1)
	except KeyboardInterrupt:
		handle_shutdown(None, None)
